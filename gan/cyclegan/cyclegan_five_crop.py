...
transform = [
    transforms.Lambda(lambda img: transforms.FiveCrop(
        int(img.size[0] / 2))(img)),
    transforms.Lambda(lambda crops: [
        transforms.Resize(int(opt.img_size), Image.BICUBIC)(crop) for crop in crops]),
    transforms.Lambda(lambda crops: [
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        for patch_from, patch_to in zip(batch['from'].squeeze(0), batch['to'].squeeze(0)):
            real_from = Tensor(patch_from.unsqueeze(0).cuda())
            real_to = Tensor(patch_to.unsqueeze(0).cuda())
